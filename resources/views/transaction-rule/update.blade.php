    ...
    <div class="content">
        <div class="container">
            <div class="page-title">
                <h3>Update Transaction Rule</h3>
            </div>
            <div class="row">
                <div class="col-lg-12">

                    <div class="card">
                        <div class="card-header">Update Transaction Rule</div>
                        <div class="card-body">
                            <form accept-charset="utf-8"
                                action="{{ route('transaction-rule.storeupdate', [$transaction->id]) }}" method="POST">
                                @csrf
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select type="text" name="account_type" class="form-control all-select2_dropdown"
                                        id="accountType">
                                        <option value = "">Select Apply To</option>
                                        <option value = "credit"
                                            {{ $transaction->account_type == 'credit' ? 'selected' : '' }}>Credit
                                        </option>
                                        <option value = "debit"
                                ...
                                <div class="mb-3">
                                    ...
                                    <select type="text" name="criteria" placeholder="Rule 0"
                                        class="form-control all-select2_dropdown">
                                        <option value="">Select Criteria</option>
                                        <option value="all_matching_criteria"
                                            {{ $transaction->transactions_category_type == 'all_matching_criteria' ? 'selected' : '' }}>
                                            All the following criteria matches
                                        </option>
                                        <option value="anyone_matching_criteria"
                                            {{ $transaction->transactions_category_type == 'anyone_matching_criteria' ? 'selected' : '' }}>
                                ...
                                @foreach ($transaction->criteria['description'] as $index => $description)
                                    <div class="mb-3 row" id="newinput">
                                        <div class="row">
                                            <div class="col-sm-3">
                                                <select class="form-control all-select2_dropdown"
                                                    name="description_type[]">
                                                    <option value="">Select Type</option>
                                                    <option value="bank_description"
                                                        {{ $description == 'bank_description' ? 'selected' : '' }}>Bank
                                                        Description</option>
                                                    <option value="our_description"
                                                        {{ $description == 'our_description' ? 'selected' : '' }}>Our
                                                        Description</option>
                                                </select>
                                            </div>
                                            <div class="col-sm-3">
                                                <select class="form-control all-select2_dropdown" name="like[]">
                                                    ...
                                                    <option value="like"
                                                        {{ $transaction->criteria['condition'][$index] == 'like' ? 'selected' : '' }}>
                                                        Like</option>
                                                </select>
                                            </div>
                                            <div class="col-sm-3">
                                                <input type="text" class="form-control" id="value" name="value[]"
                                                    value="{{ $transaction->criteria['value'][$index] }}">
                                            </div>
                                            <div class="col-sm-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select type="text" name="account" placeholder="Rule Name"
                                        class="form-control all-select2_dropdown">
                                        <option value="">Select Account</option>
                                        @foreach ($accounts as $account)
                                            <option value="{{ $account->id }}"
                                                {{ $transaction->account_name == $account->id ? 'selected' : '' }}>
                                                {{ $account->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3" id="walletAccountDiv">
                                    ...
                                    <select type="text" name="wallet_account" placeholder="Wallet Account"
                                        class="form-control all-select2_dropdown">
                                        <option value="">Select Wallet Account</option>
                                        @foreach ($wallets as $wallet)
                                            <option value="{{ $wallet->id }}"
                                                {{ $transaction->wallet_account == $wallet->id ? 'selected' : '' }}>
                                                {{ $wallet->name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3" id="paymentGatewayDiv">
                                    ...
                                    <select type="text" name="payment_account" placeholder="Payment Gateway"
                                        class="form-control all-select2_dropdown">
                                        <option value="">Select Payment Gateway Account</option>
                                        @foreach ($paymentGateways as $paymentGateway)
                                            <option value="{{ $paymentGateway->id }}"
                                                {{ $transaction->payment_account == $paymentGateway->id ? 'selected' : '' }}>
                                                {{ $paymentGateway->name }}
                                            </option>
                                        @endforeach
                                    </select>
