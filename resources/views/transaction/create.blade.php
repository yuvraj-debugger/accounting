		...
		<div class="row">
			<div class="col-lg-12">
				...
				<div class="card">
					...
					<div class="card-body">
						<form accept-charset="utf-8" action="{{route('transaction.store')}}"
							method="POST">
							@csrf
							<div class="mb-3">
								<label for="account_name" class="form-label">Date</label>
							...
							<div class="mb-3">
								...
								<select name="account_type" placeholder="Account Type"
									class="form-control all-select2_dropdown" id="account_type">
							...
							<div class="mb-3" style="display: none;" id="bank_account_show">
								...
								<select name="bank_account" placeholder="Select Bank account"
                                    ...
                                    @foreach($bankAccounts as $bankAccount)
                                        <option value="{{ $bankAccount->id}}" {{ old('bank_account') == $bankAccount->id ? 'selected' : '' }}>{{ $bankAccount->name}}</option>
                                    @endforeach
								</select>
                            ...
                            <div class="mb-3" style="display: none;" id="wallet_account_show">
								<label for="bank_account" class="form-label">Wallet Account</label>
								<select name="wallet_account" placeholder="Select Wallet account"
                                    ...
                                    @foreach($walletAccounts as $walletAccount)
                                        <option value="{{ $walletAccount->id}}" {{ old('wallet_account') == $walletAccount->id ? 'selected' : '' }}>{{ $walletAccount->name}}</option>
                                    @endforeach
								</select>

							</div>
                            <div class="mb-3" style="display: none;" id="payment_account_show">
								<label for="bank_account" class="form-label">Payment gateway Account</label>
								<select name="payment_credit" placeholder="Select Payment Gateway Account"
                                    ...
                                    @foreach($paymentWallets as $paymentWallet)
                                        <option value="{{ $paymentWallet->id}}" {{ old('payment_credit') == $paymentWallet->id ? 'selected' : '' }}>{{ $paymentWallet->name}}</option>
                                    @endforeach
								</select>
							...
							<div class="mb-3">
								...
								<textarea class="form-control" id="bank_description" name="bank_description" rows="4" cols="50">{{ old('bank_description') }}</textarea>
							...
							<div class="mb-3">
								...
								<textarea class="form-control" id="our_description" name="our_description" rows="4" cols="50">{{ old('our_description') }}</textarea>
							...
							<div class="mb-3">
								<label for="account_name" class="form-label">Amount</label>
								 <input type="number" name="amount" placeholder="Amount" class="form-control" value="{{ old('ammount') }}"/>
                                @error('amount')
								    <div class="error">{{ $message }}</div>
								@enderror
							</div>
							<div class="mb-3">
								...
								<select name="subaccount" placeholder="Select Account sub account"
                                    ...
                                    @foreach($subAccounts as $subAccount)
                                        <option value="{{ $subAccount->id}}" {{ old('subaccount') == $subAccount->id ? 'selected' : '' }}>{{ $subAccount->name}}</option>
                                    @endforeach
								</select>
							</div>
							...
							<div class="mb-3 row">
								<div class="col-sm-4 offset-sm-2">
									<a href="{{route('transaction.index')}}"
										class="btn btn-secondary mb-2"><i class="fas fa-times"></i>
										Cancel</a>
